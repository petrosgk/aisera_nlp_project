...
def infer(checkpoint, context_txt, question):
  ...
  with open(context_txt, 'r') as f:
    context = f.read()
  print(f'Context: "{context}"')
  print(f'Question: "{question}"')
  # Process context and question
  context = dataset_lib.process_string(context)
  question = dataset_lib.process_string(question)
  ...
  inputs = (np.asarray([context]), np.asarray([question]))
